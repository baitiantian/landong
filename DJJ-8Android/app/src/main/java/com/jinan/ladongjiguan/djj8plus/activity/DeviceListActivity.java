...
public class DeviceListActivity extends Activity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		title_layout.setText(R.string.select_device);
		/**
		 * 返回键
		 * */
		findViewById(R.id.examine_page_back).setOnClickListener(new OnClickListener() {
			@Override
...
//				v.setVisibility(View.GONE);
	...
	private OnItemClickListener mDeviceClickListener = new OnItemClickListener()
	{
		public void onItemClick(AdapterView<?> av, View v, int arg2, long arg3)
		{
			...
			Log.e("DeviceList","info为"+info);
			if(!info.equals("没有已连接过的设备")) {
				String address = info.substring(info.length() - 17);

				// Create the result Intent and include the MAC address
				Intent intent = new Intent();
				intent.putExtra(EXTRA_DEVICE_ADDRESS, address);
				intent.putExtra("name", info.substring(0, info.length() - 18));
